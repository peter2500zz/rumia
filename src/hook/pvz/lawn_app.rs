...
/// `LawnApp` 构造函数的签名
type SignConstructor = extern "stdcall" fn(uninit: *mut LawnApp) -> *mut LawnApp;
/// `LawnApp` 构造函数的跳板
pub static ORIGINAL_CONSTRUCTOR: OnceLock<SignConstructor> = OnceLock::new();

/// `LawnApp` 析构函数的地址
const ADDR_DESTRUCTOR: u32 = 0x0044EDF0;
/// `LawnApp` 析构函数的签名
type SignDestructor = extern "thiscall" fn(this: *mut LawnApp);
/// `LawnApp` 析构函数的跳板
pub static ORIGINAL_DESTRUCTOR: OnceLock<SignDestructor> = OnceLock::new();

/// `LawnApp::Init` 的地址
pub const ADDR_INIT: u32 = 0x00451880;
/// `LawnApp::Init` 的签名
type SignInit = extern "thiscall" fn(this: *mut LawnApp);
/// `LawnApp::Init` 的跳板
pub static ORIGINAL_INIT: OnceLock<SignInit> = OnceLock::new();

/// `LawnApp::LostFocus` 的地址
const ADDR_LOST_FOCUS: u32 = 0x0044F460;
/// `LawnApp::LostFocus` 的签名
type SignLostFocus = extern "thiscall" fn(this: *mut LawnApp);
